...
function App() {
  ...
  const handleAddTicket = (newTicket) => {
    ...
    setTickets(prevTickets => [...prevTickets, newTicket]); // Add new ticket to the state
    // console.log(tickets);
  };
        ...
        <div className='app_boards'>
          {
            group === 'status' && status.map((opt, id) => (
              <Board order={order} onAddTicket={handleAddTicket} data={opt} key={id} tickets={tickets} users={users} group={group} />
            ))
          }
          {
            group === 'user' && users.map((opt) => (
              <Board order={order} onAddTicket={handleAddTicket} data={opt} key={opt.id} tickets={tickets} users={users} group={group} userId={opt?.id} />
            ))
          }
          {
            group === 'priority' && priorities.map((opt, id) => (
              <Board order={order} onAddTicket={handleAddTicket} data={opt} level={id} key={id} tickets={tickets} users={users} group={group} />
            ))
          }
        </div>
      </div>
